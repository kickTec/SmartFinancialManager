...
public class TaskServiceImpl implements ITaskService {
    ...
    public void fourClockTask() {
        try {
            ...
            Date now = new Date();
            int weekNum = DateUtils.getWeekDayNum(now);
            int monthNum = DateUtils.getMonthDayNum(now);
            ...
            JarUtil.compressFundHistory(storageHomePath + File.separator + "history", DateUtils.getYear(new Date()));
    ...
    public void updateStockLastDate() {
        ...
        try {
            int weekNum = DateUtils.getWeekDayNum(new Date());
            if (weekNum == 6 || weekNum == 7) { // 周末跳过
                return;
            }
